...
impl Crud for Site {
  type Form = SiteForm;
  type IdType = SiteId;
  fn read(conn: &PgConnection, _site_id: SiteId) -> Result<Self, Error> {
  ...
  fn update(conn: &PgConnection, site_id: SiteId, new_site: &SiteForm) -> Result<Self, Error> {
    use crate::schema::site::dsl::*;
    diesel::update(site.find(site_id))
      .set(new_site)
      .get_result::<Self>(conn)
  }
  fn delete(conn: &PgConnection, site_id: SiteId) -> Result<usize, Error> {
    use crate::schema::site::dsl::*;
